...
int pyro_cli_cmd_bake(char* cmd_name, ArgParser* cmd_parser) {
    ...
    if (!pyro_get_embedded("std/cmd/bake.pyro", &code, &code_length)) {
        fprintf(stderr, "error: failed to load 'embed/std/cmd/bake.pyro'\n");
        pyro_free_vm(vm);
        return 1;
    }

    pyro_exec_code(vm, (const char*)code, code_length, "std::cmd::bake", NULL);
