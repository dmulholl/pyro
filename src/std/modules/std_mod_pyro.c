...
static Value fn_set_stdout(PyroVM* vm, size_t arg_count, Value* args) {
    if (!IS_FILE(args[0]) && !IS_BUF(args[0])) {
        pyro_panic(vm, "$std::pyro::set_stdout(): invalid argument [arg], expected a file or buffer");
        return MAKE_NULL();
    }
...
static Value fn_set_stderr(PyroVM* vm, size_t arg_count, Value* args) {
    if (!IS_FILE(args[0]) && !IS_BUF(args[0])) {
        pyro_panic(vm, "$std::pyro::set_stderr(): invalid argument [arg], expected a file or buffer");
        return MAKE_NULL();
    }
...
static Value fn_set_stdin(PyroVM* vm, size_t arg_count, Value* args) {
    if (!IS_FILE(args[0])) {
        pyro_panic(vm, "$std::pyro::set_stdin(): invalid argument [arg], expected a file");
        return MAKE_NULL();
    }
