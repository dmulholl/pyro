...
static Value fn_abs(PyroVM* vm, size_t arg_count, Value* args) {
    if (arg_count == 1) {
        if (IS_I64(args[0])) {
            return pyro_make_i64(imaxabs(args[0].as.i64));
        } else if (IS_F64(args[0])) {
            return pyro_make_f64(fabs(args[0].as.f64));
        }
...
static Value fn_acos(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(acos((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(acos(args[0].as.f64));
    }
...
static Value fn_asin(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(asin((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(asin(args[0].as.f64));
    }
...
static Value fn_atan(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(atan((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(atan(args[0].as.f64));
    }
...
static Value fn_cos(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(cos((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(cos(args[0].as.f64));
    }
...
static Value fn_sin(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(sin((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(sin(args[0].as.f64));
    }
...
static Value fn_tan(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(tan((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(tan(args[0].as.f64));
    }
...
static Value fn_ln(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(log((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(log(args[0].as.f64));
    }
...
static Value fn_log10(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(log10((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(log10(args[0].as.f64));
    }
...
static Value fn_log2(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(log((double)args[0].as.i64) / log(2.0));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(log(args[0].as.f64) / log(2.0));
    }
...
static Value fn_log(PyroVM* vm, size_t arg_count, Value* args) {
    ...
    return pyro_make_f64(log(operand) / log(base));
}
...
static Value fn_atan2(PyroVM* vm, size_t arg_count, Value* args) {
    ...
    return pyro_make_f64(atan2(y, x));
}


static Value fn_exp(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(exp((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(exp(args[0].as.f64));
    }
...
static Value fn_sqrt(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(sqrt((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(sqrt(args[0].as.f64));
    }
...
static Value fn_cbrt(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(cbrt((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(cbrt(args[0].as.f64));
    }
...
static Value fn_ceil(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(ceil((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(ceil(args[0].as.f64));
    }
...
static Value fn_floor(PyroVM* vm, size_t arg_count, Value* args) {
    if (IS_I64(args[0])) {
        return pyro_make_f64(floor((double)args[0].as.i64));
    } else if (IS_F64(args[0])) {
        return pyro_make_f64(floor(args[0].as.f64));
    }
...
void pyro_load_std_mod_math(PyroVM* vm, ObjModule* module) {
    pyro_define_pub_member(vm, module, "PI", pyro_make_f64(PYRO_PI));
    pyro_define_pub_member(vm, module, "E", pyro_make_f64(PYRO_E));
    pyro_define_pub_member(vm, module, "NAN", pyro_make_f64(NAN));
    pyro_define_pub_member(vm, module, "INF", pyro_make_f64(INFINITY));
    pyro_define_pub_member(vm, module, "I64_MAX", pyro_make_i64(INT64_MAX));
    pyro_define_pub_member(vm, module, "I64_MIN", pyro_make_i64(INT64_MIN));
