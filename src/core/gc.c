...
static void mark_roots(PyroVM* vm) {
    ...
    mark_object(vm, (PyroObject*)vm->str_op_binary_mod);
    mark_object(vm, (PyroObject*)vm->str_rop_binary_mod);
    mark_object(vm, (PyroObject*)vm->str_op_binary_rem);
    mark_object(vm, (PyroObject*)vm->str_rop_binary_rem);
...
static void blacken_object(PyroVM* vm, PyroObject* object) {
    ...
    switch (object->type) {
        ...
        case PYRO_OBJECT_ENUM_MEMBER: {
            ...
            mark_object(vm, (PyroObject*)enum_member->enum_type);
            mark_object(vm, (PyroObject*)enum_member->name);
            mark_value(vm, enum_member->value);
            break;
        }
    }
}
