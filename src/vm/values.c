...
// Allocates a new object on the heap and adds it to the VM's linked list of objects.
static Obj* allocate_object(PyroVM* vm, size_t size, ObjType type) {
    Obj* object = reallocate(vm, NULL, 0, size);
    object->type = type;
...
// Returns the line number in the source code corresponding to the bytecode byte at index [ip].
...
size_t ObjFn_add_constant(ObjFn* fn, Value value, PyroVM* vm) {
    // Avoid adding a duplicate entry to the table.
    ...
    // Push the value onto the stack temporarily in case the GC is triggered.
    pyro_push(vm, value);
    if (fn->constants_count == fn->constants_capacity) {
        size_t old_capacity = fn->constants_capacity;
        fn->constants_capacity = GROW_CAPACITY(old_capacity);
        fn->constants = REALLOCATE_ARRAY(vm, Value, fn->constants, old_capacity, fn->constants_capacity);
    }
    pyro_pop(vm);
