...
static int64_t pyro_write_n(PyroVM* vm, ObjFile* file, const char* string, size_t count) {
    if (!file || count == 0) {
        return 0;
    }
...
static int64_t pyro_write_fv(PyroVM* vm, ObjFile* file, const char* format_string, va_list args) {
    if (!file) {
        return 0;
    }
